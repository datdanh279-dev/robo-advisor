...
if st.session_state.trang_thai == "survey":
    ...
    if not weights:
        ...
        def _fetch_vn_bond_yield():
            import yfinance as _yf
            for sym in ["^VN10Y", "VN10Y=X", "VNI10Y=X", "VN10Y.VI"]:
                try:
            ...
            try:
                import requests as _rq, pandas as _pd
                _r = _rq.get("https://query1.finance.yahoo.com/v8/finance/chart/%5EVN10Y",
                    params={"range": "6mo", "interval": "1d"},
                    timeout=10, headers={"User-Agent": "Mozilla/5.0"})
                if _r.status_code == 200:
                    _j = _r.json()
                    _result = _j.get("chart", {}).get("result", [{}])[0]
                    _ts = _result.get("timestamp", [])
                    _cs = _result.get("indicators", {}).get("quote", [{}])[0].get("close", [])
                    _pairs = [(t, c) for t, c in zip(_ts, _cs) if c and 0 < c < 50]
                    if len(_pairs) >= 20:
                        ts_v, cs_v = zip(*_pairs)
                        return _pd.Series(list(cs_v), index=_pd.to_datetime(list(ts_v), unit="s"))
            except Exception:
                pass
            return None
        ...
        st.write("## 📑 6 NHÓM PHÂN TÍCH")
        _chon_nhom = st.selectbox("📑 Chọn nhóm phân tích:", [
        ...
        if _chon_nhom == "⚠️ Rủi ro":
            ...
            if dm and len(dm) >= 2:
                ...
                for ma, d in dm.items():
                    ...
                    if pe_v <= 0:
                        pe_v = float((DOCS.get("co_phieu_tg") or {}).get(ma, {}).get("pe", 0) or 0)
                    if pe_v > 0:
                        weighted_pe += pe_v * w
                ...
                if vn_bond_close is not None and len(vn_bond_close) > 0:
                    ...
                    if 0 < bv < 1:
                        ...
                        bond_yield_now = bv
                if bond_yield_now <= 0:
                    for _sym in ["^VN10Y", "VN10Y=X", "VNI10Y=X", "VN10Y.VI"]:
                        try:
                            import yfinance as _yf_b
                            _b = _yf_b.Ticker(_sym).history(period="1mo", timeout=8)
                            if not _b.empty:
                                bv = float(_b['Close'].dropna().iloc[-1])
                                if 0 < bv < 1:
                                    bond_yield_now = bv * 100
                                elif 1 <= bv < 50:
                                    bond_yield_now = bv
                ...
                if bond_yield_now <= 0:
                    try:
                        ...
                        _r = _rq_b.get("https://query1.finance.yahoo.com/v8/finance/chart/%5EVN10Y",
                            params={"range": "1mo", "interval": "1d"},
                            timeout=10, headers={"User-Agent": "Mozilla/5.0"})
                        if _r.status_code == 200:
                            _j = _r.json()
                            _cs = _j.get("chart", {}).get("result", [{}])[0].get("indicators", {}).get("quote", [{}])[0].get("close", [])
                ...
                if 0 < bond_yield_now < 30 and _ey_count > 0:
                    ...
                    fw = " và co_phieu_vn/tg.json" if _ey_count > 0 else ""
